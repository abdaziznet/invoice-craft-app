...
export const generatePdfFlow = ai.defineFlow(
  {
  ...
  async ({ invoiceId }) => {
    ...
    metaInfo.forEach(info => {
        ...
        page.drawText(info.value, { x: metaX + 80, y: currentMetaY, font: font, size: fontSize, color: rgb(0.3, 0.3, 0.3)});
        currentMetaY -= 20;
    });
    ...
    const summaryItems = [
        ...
        { label: `Tax (${invoice.tax}%)`, value: formatCurrency((invoice.subtotal * invoice.tax) / 100) },
    ];
    ...
    summaryItems.forEach(item => {
        ...
        page.drawText(item.value, { x: summaryValueX, y: y, font: font, size: fontSize, align: 'right' });
        y -= 20;
    });
    ...
    page.drawText(formatCurrency(invoice.total), { x: summaryValueX, y: y, font: boldFont, size: subHeaderFontSize, align: 'right' });
    ...
    if(invoice.notes) {
        ...
        notesLines.forEach(line => {
            page.drawText(line, {x: notesX, y: currentNotesY, font: font, size: smallFontSize, color: rgb(0.3, 0.3, 0.3)});
            currentNotesY -= 12;
        });
    }
